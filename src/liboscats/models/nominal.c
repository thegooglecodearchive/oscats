...
G_DEFINE_TYPE(OscatsIrtModelNominal, oscats_irt_model_nominal, OSCATS_TYPE_IRT_MODEL);
...
static void oscats_irt_model_nominal_class_init (OscatsIrtModelNominalClass *klass)
{
...
static void oscats_irt_model_nominal_init (OscatsIrtModelNominal *self)
{
}
...
static void model_constructed(GObject *object)
{
  ...
  G_OBJECT_CLASS(oscats_irt_model_nominal_parent_class)->constructed(object);
  Ncat = OSCATS_IRT_MODEL_NOMINAL(model)->Ncat;
...
static void model_set_property(GObject *object, guint prop_id,
                               const GValue *value, GParamSpec *pspec)
{
  OscatsIrtModelNominal *self = OSCATS_IRT_MODEL_NOMINAL(object);
  switch (prop_id)
  {
                          ...
                          GGslVector *grad, GGslMatrix *hes, gboolean Inf)
{
  ...
  guint i, j, I, J, x, y, Ncat = ((OscatsIrtModelNominal*)model)->Ncat;
                          ...
                          GGslVector *grad, GGslMatrix *hes)
{
  gsl_vector *grad_v = (grad ? grad->v : NULL);
  gsl_matrix *hes_v = (hes ? hes->v : NULL);
  guint Ncat = ((OscatsIrtModelNominal*)model)->Ncat;
